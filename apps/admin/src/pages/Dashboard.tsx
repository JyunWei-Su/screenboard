...
export default function Dashboard() {
  ...
  return <div className="space-y-6">
    ...
    <div className="space-y-3"><h2 className="card-title">未處理事件</h2>
      ...
      {eventPage && <div className="flex items-center justify-between text-sm text-slate-500"><span>共 {eventPage.total} 筆，第 {eventPage.page} / {eventPage.total_pages} 頁</span><div className="flex gap-2"><button className="btn-ghost" disabled={page <= 1} onClick={() => setPage(page - 1)}>上一頁</button><button className="btn-ghost" disabled={page >= eventPage.total_pages} onClick={() => setPage(page + 1)}>下一頁</button></div></div>}
    </div>
  </div>;
}
