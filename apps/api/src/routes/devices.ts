...
app.get("/:uuid/remote-access", requireRole("admin", "operator"), async (c) => {
  ...
  const row = await c.env.DB.prepare(
    "SELECT tunnel_id, hostname, access_app_id, status, last_error, updated_at, provisioning_version, ssh_ca_public_key FROM device_remote_access WHERE device_id = ?",
  ).bind(uuid).first<{ tunnel_id: string; hostname: string; access_app_id: string | null; status: string; last_error: string | null; updated_at: string; provisioning_version: number; ssh_ca_public_key: string | null }>();
  ...
  return c.json({
    ...
    needs_reprovision: row.hostname !== hostnameFor(c.env, uuid) || row.provisioning_version < 3,
  });
});
...
app.post("/:uuid/remote-access", requireRole("admin"), async (c) => {
  ...
  const needsReplacement = Boolean(
    existing && (existing.hostname !== hostnameFor(c.env, uuid) || existing.provisioning_version < 3),
  );
